...
const DependencyTree = goog.require(
	"proto.build.stack.bazel.registry.v1.DependencyTree",
);
const DependencyTreeNode = goog.require(
	"proto.build.stack.bazel.registry.v1.DependencyTreeNode",
);
...
class MVS {
	...
	buildTree(moduleVersion, visited, selected, includeDev) {
		...
		for (const dep of deps) {
			...
			let overrideType = "";
			if (dep.getOverride()) {
				overrideType = this.getOverrideType_(
					asserts.assertObject(dep.getOverride()),
				);
				// For single_version_override, use the override version
				if (dep.getOverride().hasSingleVersionOverride()) {
					const overrideVersion = dep
						.getOverride()
						.getSingleVersionOverride()
						.getVersion();
					if (overrideVersion) {
			...
			if (!selectedModuleVersion) {
				console.warn(
					`Module version not found: ${depModuleName}@${requestedVersion}`,
				);
				continue;
			}
			...
			const grandchildren = this.buildTree(
	...
	flatten(nodes, selected) {
		for (const node of nodes) {
			...
			if (!current) {
				selected.set(
					nodeModuleName,
					asserts.assertObject(node.getModuleVersion()),
				);
			} else {
				const comparison = this.compareVersions_(
					nodeModuleName,
	...
	checkDirectDeps_(rootModule, selected) {
		for (const dep of rootModule.getDepsList()) {
			...
			if (
				selectedVersion &&
				dep.getVersion() !== selectedVersion.getVersion()
			) {
				console.warn(
	...
	compareVersions_(moduleName, v1, v2) {
		...
		if (!metadata) {
			...
			console.warn(
				`No metadata found for module: ${moduleName}, falling back to string comparison`,
			);
			return v1 < v2 ? -1 : 1;
		}
		...
		if (index1 === -1 || index2 === -1) {
			console.warn(
				`Version not found in metadata for ${moduleName}: v1=${v1} (index=${index1}), v2=${v2} (index=${index2})`,
			);
			return v1 < v2 ? -1 : 1;
		}
