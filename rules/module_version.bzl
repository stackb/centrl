...
def _make_moduleversion(name, version, deps, starlarklibrary):
    return struct(
        ...
        name = name,
        version = version,
        deps = {_dep_module_dependency_repo_name(dep): dep for dep in deps.to_list()},
        repo_name = starlarklibrary.repo_name,
        starlarklibrary = starlarklibrary,
        unknown_loads = [],
    )

def _compile_best_effort_extract_documentation_action(ctx, source, deps):
    # output = ctx.actions.declare_file(ctx.label.name + ".moduleinfo.pb")
    ...
    # bazel_tools_bundle = ctx.attr._bazel_tools_docs[StarlarkLibraryFileInfo]
    # rules_cc_bundle = ctx.attr._rules_cc_docs[StarlarkLibraryFileInfo]

    # DepSet[ModuleDependencyInfo] - transitive depset of module dependencies
    transitive_module_deps = depset(deps, transitive = [d.module.deps for d in deps if getattr(d, "module", None)])
    ...
    moduleversions = [
        ...
        for dep in transitive_module_deps.to_list()
        if getattr(dep.module, "source", None)
    ] + [
        _make_moduleversion(ctx.attr.module_name, ctx.attr.version, depset(deps), source.starlarklibrary),
        _make_moduleversion("bazel_tools", "", depset([]), ctx.attr._bazel_tools[StarlarkLibraryFileInfo]),
    ]
    ...
    transitive_srcs = [depset(moduleversion.starlarklibrary.srcs) for moduleversion in moduleversions]
    ...
    # for module, provider in transitive_starlarklibrary:
    #     for dep in module.deps.to_list():
    #         args.add("--module_dep=%s=%s:%s:%s" % (provider.repo_name, dep.name, dep.repo_name, dep.version))
    #     for file in provider.srcs:
    #         args.add("--bzl_file=%s=%s" % (provider.repo_name, file.path))
...
def _compile_published_documentation_action(ctx, files):
    ...
    output = ctx.actions.declare_file(ctx.label.name + ".docs.json")
...
def _compile_docs_action(ctx, source, deps, unresolved_deps):
    ...
    return _compile_best_effort_extract_documentation_action(ctx, source, deps)
